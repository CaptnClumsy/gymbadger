...
function resetMarkers() {
  ...
  function initRaidBosses() {
	  $.ajax({
          ...
          success: function (data) {
      	    ...
      	    initS2Cells();
      	    showGymFromUrl();
      	    selectLevel(cellLevel, showCells);
      	    initHistoryEdit();
          },
          error: function (result) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.total==null || data.total==0) {
            ...
            subTitle = ""+data.total+" raids ";
        }
        if (data.since!=null) {
          var since = new Date(data.since);
          $('#favouritesSubTitle').html("<p class=\"badger-fav-sub-title\">"+subTitle+"since "+since.toDateString()+"</p>");
        } else {
          $('#favouritesSubTitle').html("<p class=\"badger-fav-sub-title\">"+subTitle+"</p>");
        }
        if (data.total!=null && data.total!=0) {
  ...
  function loadS2Selection() {
	  if (typeof(Storage) !== "undefined") {
		  var showCellsStorage = localStorage.getItem("showCells");
		  if (showCellsStorage !== null) {
			  showCells = (showCellsStorage=='true');
		  } else {
			  showCells = false;
		  }
		  var cellLevelStorage = localStorage.getItem("cellLevel");
		  if (cellLevelStorage !== null) {
	          cellLevel = cellLevelStorage;
		  } else {
			  cellLevel = 17;
		  }
	  }
  ...
  function queryS2Cells() {
	  ...
	  $.ajax({
	      type: "GET",
	      contentType: "application/json; charset=utf-8",
	      url: "/api/cells/"+cellLevel+"/"+centerlat+"/"+centerlng+"/",
	      success: function (data) {
	    	  for (var i=0; i<data.length; i++) {
